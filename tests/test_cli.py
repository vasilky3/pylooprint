...
def test_writes_a_valid_3mf(cone_multi_project, tmp_path, capsys):
    destination = tmp_path / "looped.gcode.3mf"
    assert main([str(cone_multi_project), "-n", "2", "-o", str(destination)]) == 0
    ...
    with zipfile.ZipFile(destination) as archive:
        plate = next(name for name in archive.namelist() if name.endswith(".gcode"))
        gcode = archive.read(plate).decode("utf-8")
        assert "Metadata/slice_info.config" in archive.namelist()
    assert gcode.count("; >>> LOOP 2 / 2 <<<") == 1


def test_refuses_an_already_looped_file(cone_multi_project, capsys, tmp_path):
    looped = tmp_path / "looped.gcode.3mf"
    assert main([str(cone_multi_project), "-o", str(looped)]) == 0

    exit_code = main([str(looped), "-n", "2", "-o", str(tmp_path / "twice.gcode.3mf")])
    assert exit_code == 1
    assert "already been looped" in capsys.readouterr().err


def test_dry_run_writes_nothing(cone_multi_project, tmp_path, capsys):
    destination = tmp_path / "nope.gcode.3mf"
    assert main([str(cone_multi_project), "-n", "2", "--dry-run", "-o", str(destination)]) == 0
...
def test_rejects_out_of_range_settings(cone_multi_project, capsys):
    assert main([str(cone_multi_project), "-n", "0"]) == 1
    assert "--loops must be between" in capsys.readouterr().err
    assert main([str(cone_multi_project), "-t", "200"]) == 1
    assert "--temp must be between" in capsys.readouterr().err


def test_reports_a_risky_cooldown_temperature(cone_multi_project, tmp_path, capsys):
    main([str(cone_multi_project), "-n", "1", "-t", "40", "--dry-run", "-o", str(tmp_path / "x.3mf")])
    assert "may not release the part" in capsys.readouterr().err


def test_the_default_cooldown_is_26_degrees(cone_multi_project, tmp_path, capsys):
    main([str(cone_multi_project), "-n", "1", "--dry-run", "-o", str(tmp_path / "x.3mf")])
    # The A1 Mini's bed sensor reads 4 degrees high, so 26 is commanded as 22.
    assert "cool-down   : 26 C -> commanded 22 C" in capsys.readouterr().out


def test_the_model_box_is_reported(cone_multi_project, tmp_path, capsys):
    main([str(cone_multi_project), "--dry-run", "-o", str(tmp_path / "x.3mf")])
    assert "model box   : X 30.6..147.9  Y 9.6..103.8" in capsys.readouterr().out
