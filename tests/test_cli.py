...
def test_writes_a_valid_3mf(result_3mf, tmp_path, capsys):
    destination = tmp_path / "looped.gcode.3mf"
    assert main([str(result_3mf), "-n", "2", "--force", "-o", str(destination)]) == 0
...
def test_dry_run_writes_nothing(result_3mf, tmp_path, capsys):
    destination = tmp_path / "nope.gcode.3mf"
    assert main([str(result_3mf), "-n", "2", "--force", "--dry-run", "-o", str(destination)]) == 0
...
def test_reports_a_risky_cooldown_temperature(result_3mf, tmp_path, capsys):
    main([str(result_3mf), "-n", "1", "-t", "40", "--force", "--dry-run", "-o", str(tmp_path / "x.3mf")])
    assert "may not release the part" in capsys.readouterr().err


def test_printer_can_be_overridden(result_3mf, tmp_path, capsys):
    main([str(result_3mf), "-n", "1", "-p", "a1", "--force", "--dry-run", "-o", str(tmp_path / "x.3mf")])
